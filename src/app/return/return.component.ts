...
export class ReturnComponent implements OnInit {
  ...
  fabricU: any;
   ...
   searchsupplier = 'Search By QUotation No';
   piNo: any = 0;
   go = false;
   ...
   branch: any;
   arreClients: any;
   serchClient: any = 'Search By Customer';
   dateFrom: any = 'All';
   dateTo: any;
  ...
  constructor(private _ApiService: ApiService , private _Router: Router, private _AppComponent: AppComponent ) {
    ...
    this._AppComponent.activeHRE = 'active';
  ...
  addS0(data: any){
    ...
    this._ApiService.addI(formdata).subscribe((res)=>{
      if(res.check === true){
        this._ApiService.allReturn().subscribe((res0)=>{
          if(res0.check === true){
            this.arre = res0.data;
            let x = this.arre;
  ...
  ngOnInit(): void {
    ...
    // setTimeout(() => {
      ...
      $('.serchClient').on('select2:select', function (e: any) {
        self.serchClient = $('.serchClient').val()
      });
