...
@Component({
  selector: 'app-client-visetor',
  templateUrl: './client-visetor.component.html',
  styleUrls: ['./client-visetor.component.scss']
})
export class ClientVisetorComponent implements OnInit {
  ...
  constructor(private _ApiService: ApiService , private _Router: Router, private _AppComponent: AppComponent, private _Route: ActivatedRoute ) {
    ...
    this._AppComponent.activeCVV = 'active';
    ...
    this._ApiService.allSC0Visit().subscribe((res)=>{
      if(res.check === true){
        this.arre = res.data;
  ...
  addS0(data: any){
    ...
    this._ApiService.addSCN(data).subscribe((res)=>{
      if(res.check === true){
        this._ApiService.addSCNAnother(data).subscribe((res)=>{
          if(res.check === true){
            this._ApiService.allSC0Visit().subscribe((res0)=>{
              if(res0.check === true){
                this.arre = res0.data;
  ...
  printQWE(data: any){
    this.DatePrint = data.date;
    this.TimePrint = data.time;
    this.ShiftPrint = data.shift;
    this.ClientPrint = data.name;
    this.OccupationPrint = data.occupation;
    this.AdressPrint = data.adress;
    this.MobilePrint = data.mobile;
    this.EmailPrint = data.email;
    this.AoutPrint = data.social;
    this.InterestPrint = data.interested;
    this.NotesPrint = data.notes;
    $('.asdfg').css({display: 'block'});
    $('.noer').css({display: 'none'});
    setTimeout(() => {
      window.print();
      $('.asdfg').css({display: 'none'});
    $('.noer').css({display: 'block'});
    }, 400);
