...
export class SheetComponent implements OnInit {
  ...
  constructor(private _ApiService: ApiService , private _Router: Router, private _AppComponent: AppComponent ) {
    ...
    this._ApiService.allCustomerOffer().subscribe((res)=>{
      if(res.check === true){
        this.arre = res.data;
        this.ngOnInit();
      } else{
  ...
  okUp0(data: any){
    ...
    this._ApiService.updateSCR0Offer(data).subscribe((res)=> {
      if(res === true) {
        this._ApiService.allCustomerP0Offer({id: $('.select2Q').val()}).subscribe((res)=>{
          this.amount = 0;
  ...
  ok(){
    ...
    var x = this.idDS;
    var e = this.nameD;
    this._ApiService.delSC0Offer({id: x , name: this.nameD , qty: this.qty , id_item: this.id_item}).subscribe((res)=>{
      if(res === true){
        this._ApiService.allCustomerP0Offer({id: $('.select2Q').val()}).subscribe((res)=>{
          this.arre00 = res.data;
          $('#gotohill').trigger('click');
  ...
  async deleteall(){
    ...
    this._ApiService.delAllOffer({id: $('.select2Q').val()}).subscribe((res)=>{
      if(res == true){
        this._ApiService.allCustomerOffer().subscribe((res)=>{
          if(res.check === true){
            this.arre = res.data;
    ...
    if($('.select2Q').val() == '' || $('.select2Q').val() == undefined ||$('.selectPlus').val() == '' || $('.selectPlusR').val() == '' || this.qty == '' || $('.selectPlus').val() == undefined || $('.selectPlusR').val() == undefined || this.qty == undefined) {
      ...
      this.allI.forEach((element: any) => {
        if($('.selectPlus').val() == element.code && $('.selectPlusR').val() == element.place) {
          ...
          this._ApiService.addS0CBOffer(data).subscribe((res)=>{
            if(res.check === true){
            this._ApiService.allCustomerP0Offer({id: $('.select2Q').val()}).subscribe((res0)=>{
            if(res0.check === true){
              this.amount = 0;
        ...
        self.amount = 0;
        $('.loading').css({display: 'block'});
        self._ApiService.allCustomerPOffer({id: $('.select2Q').val()}).subscribe((res)=>{
          if(res.check === true){
            ...
            self._ApiService.allCustomerP0Offer({id: $('.select2Q').val()}).subscribe((res0)=>{
              if(res0.check === true){
                res0.data.forEach((element: any) => {
  ...
  addS0C(data: any){
    if($('.goQN').val() == '' || $('.goQN').val() == undefined ) {
      ...
      this.addCu()
      $('.loading').css({display: 'block'});
      this._ApiService.addSCOffer({name: $('.goQN').val() , seller: $('.goQN00').val() , broker: $('.goQN00X0').val(),brokerCommision: this.commision }).subscribe((res)=>{
        if(res.check === true){
        this._ApiService.allCustomerOffer().subscribe((res0)=>{
        if(res0.check === true){
        ...
        this._ApiService.addPay(formdata).subscribe((res)=>{
          if(res === true) {
            this._ApiService.getItemBooked({id: data.id}).subscribe((res0)=>{
              this._ApiService.allCustomerPOffer({id: data.id}).subscribe((res1)=>{
                if(res1.check === true){
  ...
  upN5(){
    ...
    let ff = $('.select2Q').val();
      if(this.nameU == '' || this.nameU == undefined ) {
        ...
        $('.loading').css({display: 'block'});
      this._ApiService.updateSC0AOffer({id: $('.select2Q').val(),pay: this.payAll,discound: this.discoundAll , commssionbroker: this.commision, broker_id: $('.goQN00X0').val(), seller_id: $('.goQN00').val() , dateAm: $('#goDate').val()}).subscribe((res)=> {
        if(res === true) {
          this._ApiService.allSC0().subscribe((res)=>{
            this.arreCustomer = res.data;
            // this._ApiService.allCustomerOffer().subscribe((res)=>{
  ...
  items0Order(){
    $('.loading').css({display: 'block'});
    this._ApiService.toquotion({id: $('.select2Q').val()}).subscribe(async (res)=> {
        ...
        self._ApiService.allCustomerPOffer({id: $('.select2Q').val()}).subscribe((res)=>{
          if(res.check === true){
            ...
            self._ApiService.allCustomerP0Offer({id: $('.select2Q').val()}).subscribe((res0)=>{
              if(res0.check === true){
                let t = 0;
                res0.data.forEach((element: any , index: any) => {
                  self.amount = self.amount + element.pay
                  if(element.qty > element.qtyfind){
                    t++;
                    setTimeout(() => {
                      $(`#d${index}`).css({display: 'inline-block'});
                    }, 100);
                  }
                });
                if(t > 0) {
                  $('#dis00').css({display: 'none'});
                } else {
                  $('#dis00').css({display: 'block'});
                }
                self.arre00 = res0.data;
                self.payAll0 = self.amount - (self.amount * ( self.discoundAll / 100));
                $('.select2QC').val(res.data[0].customer_id).trigger('change.select2');
